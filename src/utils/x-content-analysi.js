  ...
  for (let i = 1; i < headingLevels.length; i++) {
    if (headingLevels[i] > 0) {
      if (i - lastNonZeroIndex > 1) {
        errors.push(
          `Skipped heading level: H${lastNonZeroIndex + 1} to H${i + 1}`,
        );
      }
      lastNonZeroIndex = i;
    }
...
const formatContentAnalysisResult = (result) => {
  ...
  const fullText = `${result.title} ${result.metaDescription} ${result.h1} ${
    result.pageData?.bodyText || ''
  }`;
  const keywords = extractKeywords(fullText);
  const headingErrors = detectHeadingErrors(result);

  const imagesWithoutAlt = result.images
    ? result.images
  ...
  return {
    ...
    imagesWithoutAlt: result.images
      ? result.images.filter((img) => !img.alt || img.alt.trim() === '').length
      : 0,
    jsErrors: Array.isArray(result.jsErrors) ? result.jsErrors.length : 0,
    schemaTypes: Array.isArray(result.structuredData)
    ...
    daysSinceLastCrawled:
      result.contentFreshness?.daysSinceLastCrawled || 'Unknown',
...
async function saveContentAnalysis(results, outputDir) {
  ...
  if (Array.isArray(results.contentAnalysis)) {
    ...
    && typeof results.contentAnalysis === 'object'
  ) {
    formattedResults = [formatContentAnalysisResult(results.contentAnalysis)];
  } else {
    console.warn(
      'results.contentAnalysis is neither an array nor an object. No content analysis will be saved.',
    );
  }

  const contentAnalysisCsv = formatCsv(formattedResults, headers);

  const imagesWithoutAlt = formattedResults.flatMap(
    (result) => result.imagesWithoutAlt,
  );
  const imagesWithoutAltHeaders = ['url', 'src', 'location'];
  const imagesWithoutAltCsv = formatCsv(
    imagesWithoutAlt,
    imagesWithoutAltHeaders,
  );
