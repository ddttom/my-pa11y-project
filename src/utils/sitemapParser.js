...
export async function fetchAndParseSitemap(sitemapPath) {
  ...
  try {
    ...
    console.error(
      `Error fetching or parsing content from ${sitemapPath}:`,
      error.message,
    );
    throw error;
  }
...
export async function extractUrls(parsedContent) {
  if (parsedContent.html) {
    ...
    if (parsedContent.xml.sitemapindex) {
      ...
      const sitemapUrls = parsedContent.xml.sitemapindex.sitemap.map(
        (sitemap) => ({
      ...
      for (const sitemapUrl of sitemapUrls) {
        ...
        try {
          const nestedParsedContent = await fetchAndParseSitemap(
            sitemapUrl.url,
          );
          ...
          console.error(
            `Error processing nested sitemap ${sitemapUrl.url}:`,
            error.message,
          );
        }
      }
      ...
      return parsedContent.xml.urlset.url
        .map((url) => ({
